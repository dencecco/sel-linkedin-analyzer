...
for key, label in zip(
    ...
    default_idx = options.index(map_cols[key]) if map_cols.get(key) in options else 0
    map_cols[key] = st.sidebar.selectbox(label, options, index=default_idx, key=key)

# Validate required mappings
required = ["likes","replies","reposts","author"]
if any(map_cols[k] is None for k in required):
...
def enrich(df):
    ...
    df["total_interactions"] = (
        df[map_cols["likes"]] + df[map_cols["replies"]] + df[map_cols["reposts"]]
    )
    # Compute engagement rate
    if map_cols.get("views") and map_cols["views"] in df.columns:
        df["eng_rate_%"] = (
            df["total_interactions"] / df[map_cols["views"]].replace({0: pd.NA}) * 100
        )
    else:
        df["eng_rate_%"] = pd.NA
    # Parse timestamps
...
if not df_comp.empty:
    if map_cols.get("author") in df_comp.columns:
...
if not df_comp.empty:
    with pages[idx["Compare"]]:
        ...
        # Include views & engagement if available
        if map_cols.get('views') and map_cols['views'] in combined.columns:
            view_means = combined.groupby('brand')[map_cols['views']].mean()
            eng_means = combined.groupby('brand')['eng_rate_%'].mean()
            agg['avg_views'] = agg['brand'].map(view_means)
            agg['avg_eng_%'] = agg['brand'].map(eng_means)
...
with pages[idx["Top 10"]]:
    ...
    # Clickable Tweet links
    if map_cols.get('url') and map_cols['url'] in top10.columns:
        top10['Tweet'] = top10.apply(
            lambda r: f"<a href='{r[map_cols['url']]}' target='_blank'>{str(r[map_cols['content']])[:80]}</a>", axis=1
        )
        display_col = 'Tweet'
    else:
        display_col = map_cols['content']
    cols = [display_col, 'date_time', map_cols['likes'], map_cols['replies'], map_cols['reposts'], 'total_interactions']
    if map_cols.get('views'):
        cols += [map_cols['views'], 'eng_rate_%']
    st.write(top10[cols].to_html(escape=False), unsafe_allow_html=True)
